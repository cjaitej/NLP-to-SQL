...
Before writing the SQL query, follow these steps:

1. Identify all relevant tables and their relationships (joins, primary/foreign key links).

2. Determine which columns are needed to answer the question.

3. Plan any necessary filters, groupings, or aggregations.

4. Construct the logical query plan in plain English.

5. Write the final SQL query based on that plan.



SQL Formatting Rules:

- You may format the SQL using indentation and line breaks for readability.

- Always use **explicit JOINs** for multi-table queries.

- Include **only the columns** explicitly required by the question.

- Ensure the SQL syntax is valid for **SQLite**.

- The SQL should be **clear, correct, and minimal**, not overly optimized or verbose.

- If any assumptions are required (due to missing schema info), clearly state them in your reasoning.
...
WHERE …"""
    ...
    sub_path = os.path.join("spider", "dev_subset.json")
